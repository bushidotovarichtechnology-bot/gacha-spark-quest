...
BEGIN
  ...
  FOR _i IN 1.._draw_count LOOP
    ...
    IF _is_pity_draw THEN
      -- PITY: pick a tier that EXACTLY matches the configured pity tier
    ...
    IF _sel_tier_id IS NULL THEN
      -- Pity disabled, not yet triggered, or pity tier has no stock → normal weighted roll
      _is_pity_draw := false;
      WITH active_tiers AS (
        SELECT ct.id, ct.label, ct.probability_weight
          FROM public.campaign_tiers ct
    ...
    -- Pick exactly ONE prize from the selected tier
    ...
    -- Decrement exactly 1 unit (or refill if depleted)
