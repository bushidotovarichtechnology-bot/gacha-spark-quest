...
DECLARE
  ...
  _exclude_s boolean;
  -- Akumulasi: berapa banyak hadiah tier C yang sudah terambil di campaign ini
  _c_consumed integer := 0;
  -- Bonus per 1 prize C terambil (basis: probability_weight). Karena sumber kebenaran UI = persen,
  -- 0.01% direpresentasikan sebagai weight 0.01; 0.001% sebagai 0.001.
  _boost_c numeric := 0.01;    -- +0.01% per 1 prize C terambil
  _boost_ba numeric := 0.001;  -- +0.001% per 1 prize C terambil (tier B & A)
  _bonus_c numeric;
  _bonus_ba numeric;
BEGIN
  ...
  FOR _i IN 1.._draw_count LOOP
    ...
    -- Hitung jumlah hadiah tier C yang sudah terambil = SUM(total - remaining) untuk tier label 'C'.
    -- Reset otomatis: saat admin refill (remaining = total), nilai ini = 0.
    ...
    _bonus_c  := _c_consumed * _boost_c;   -- bonus weight untuk tier C
    _bonus_ba := _c_consumed * _boost_ba;  -- bonus weight untuk tier A & B (sangat kecil)
    ...
    IF _sel_tier_id IS NULL THEN
      ...
      WITH active_tiers AS (
        SELECT ct.id, ct.label,
               -- Bobot efektif = bobot dasar + bonus akumulasi sesuai label
