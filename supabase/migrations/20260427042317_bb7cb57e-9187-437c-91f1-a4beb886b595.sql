-- 1) Add review_expires_at column to trades
ALTER TABLE public.trades
  ADD COLUMN IF NOT EXISTS review_expires_at timestamptz;

-- 2) Replace _internal_execute_trade with a 2-step flow
DROP FUNCTION IF EXISTS public._internal_execute_trade(uuid, uuid, jsonb, text, text);
...
DECLARE
  ...
  _gas_fee constant integer := 1;
  _new_initiator_balance integer;
  _new_responder_balance integer;
  _exchanged jsonb;
  _review_window constant interval := interval '1 hour';
BEGIN
  IF _action NOT IN ('submit', 'approve', 'reject') THEN
  ...
  IF _trade.tier_label NOT IN ('S','A','B') THEN
    RAISE EXCEPTION 'tier_locked' USING ERRCODE = 'P0001';
  END IF;

  -- Auto-expire stale trades on read
  IF _trade.status = 'pending' AND _trade.expires_at IS NOT NULL AND _trade.expires_at < now() THEN
  ...
  END IF;
  IF _trade.status = 'awaiting_initiator' AND _trade.review_expires_at IS NOT NULL AND _trade.review_expires_at < now() THEN
    UPDATE public.trades SET status = 'expired', updated_at = now() WHERE id = _trade_id;
    RAISE EXCEPTION 'review_window_expired' USING ERRCODE = 'P0001';
  END IF;

  ----------------------------------------------------------------
  -- STEP 1: Responder submits items + PIN -> awaiting_initiator
  ----------------------------------------------------------------
  IF _action = 'submit' THEN
    IF _trade.status <> 'pending' THEN
      RAISE EXCEPTION 'trade_not_pending' USING ERRCODE = 'P0001';
    END IF;
    IF _caller_id = _trade.initiator_id THEN
      RAISE EXCEPTION 'self_trade_forbidden' USING ERRCODE = 'P0001';
    END IF;
    ...
    -- Validate ownership + tier of responder items now (early failure)
    SELECT array(SELECT (jsonb_array_elements_text(_responder_items))::uuid) INTO _responder_items_arr;

    SELECT count(*) INTO _responder_owned_count FROM public.user_inventory
      WHERE id = ANY(_responder_items_arr) AND user_id = _caller_id;
    IF _responder_owned_count <> array_length(_responder_items_arr, 1) THEN
    ...
    IF _responder_tier_match <> array_length(_responder_items_arr, 1) THEN
      RAISE EXCEPTION 'tier_mismatch' USING ERRCODE = 'P0001';
    END IF;

    UPDATE public.trades
       SET responder_id = _caller_id,
           responder_items = _responder_items,
           status = 'awaiting_initiator',
           review_expires_at = now() + _review_window,
           updated_at = now()
     WHERE id = _trade_id
    ...
    RETURN jsonb_build_object(
      ...
      'trade_id', _trade.id,
      'review_expires_at', _trade.review_expires_at
    );
  END IF;

  ----------------------------------------------------------------
  -- STEP 2a: Initiator REJECTS -> trade cancelled
  ----------------------------------------------------------------
  IF _action = 'reject' THEN
    IF _trade.status <> 'awaiting_initiator' THEN
      RAISE EXCEPTION 'trade_not_pending' USING ERRCODE = 'P0001';
    END IF;
    IF _caller_id <> _trade.initiator_id THEN
    ...
    INSERT INTO public.trade_history(
      ...
      _trade.id, _trade.initiator_id, _trade.responder_id, _trade.tier_label,
      jsonb_build_object('initiator_items', _trade.initiator_items, 'responder_items', _trade.responder_items, 'tier', _trade.tier_label),
      0, 'rejected_by_initiator', 'initiator_rejected_responder_items',
      _trade.initiator_ip, _caller_ip, COALESCE(_user_agent, _trade.initiator_user_agent)
    );

    RETURN jsonb_build_object('success', true, 'stage', 'rejected', 'trade_id', _trade.id);
  END IF;

  ----------------------------------------------------------------
  -- STEP 2b: Initiator APPROVES -> execute the swap
  ----------------------------------------------------------------
  -- _action = 'approve'
  IF _trade.status <> 'awaiting_initiator' THEN
    RAISE EXCEPTION 'trade_not_pending' USING ERRCODE = 'P0001';
  ...
  RETURN jsonb_build_object(
    'success', true, 'stage', 'accepted', 'trade_id', _trade.id, 'tier', _trade.tier_label,
...
-- 3) Update expire_stale_trades to also expire trades stuck in review
...
BEGIN
  ...
  UPDATE public.trades
     SET status = 'expired', updated_at = now()
   WHERE status = 'awaiting_initiator'
