...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'trade_not_found' USING ERRCODE = 'P0001';
  END IF;
  ...
  -- Caller must be responder (or claim the open link).
  IF _trade.responder_id IS NULL THEN
    -- Claim: caller becomes responder if not the initiator.
    IF _caller_id = _trade.initiator_id THEN
      RAISE EXCEPTION 'self_trade_forbidden' USING ERRCODE = 'P0001';
    END IF;
    -- Need responder items to proceed.
  ...
  ELSE
    -- Already responder; allow updating items if provided.
    IF _responder_items IS NOT NULL AND jsonb_array_length(_responder_items) > 0 THEN
      UPDATE public.trades
         SET responder_items = _responder_items,
             updated_at = now()
       WHERE id = _trade_id
       RETURNING * INTO _trade;
    END IF;
  ...
  -- Convert jsonb arrays to uuid[].
  SELECT array(SELECT (jsonb_array_elements_text(_trade.initiator_items))::uuid)
    INTO _initiator_items_arr;
  SELECT array(SELECT (jsonb_array_elements_text(_trade.responder_items))::uuid)
    INTO _responder_items_arr;

  IF array_length(_initiator_items_arr, 1) IS NULL OR array_length(_responder_items_arr, 1) IS NULL THEN
    RAISE EXCEPTION 'missing_responder_items' USING ERRCODE = 'P0001';
  END IF;

  -- Lock & validate ownership + tier.
  PERFORM 1 FROM public.user_inventory
   WHERE id = ANY(_initiator_items_arr) FOR UPDATE;
  PERFORM 1 FROM public.user_inventory
   WHERE id = ANY(_responder_items_arr) FOR UPDATE;

  SELECT count(*) INTO _initiator_owned_count
    FROM public.user_inventory
   WHERE id = ANY(_initiator_items_arr) AND user_id = _trade.initiator_id;
  SELECT count(*) INTO _responder_owned_count
    FROM public.user_inventory
  ...
  -- Tier equality on every offered item.
  SELECT count(*) INTO _initiator_tier_match
    FROM public.user_inventory
   WHERE id = ANY(_initiator_items_arr) AND tier_label = _trade.tier_label;
  SELECT count(*) INTO _responder_tier_match
    FROM public.user_inventory
  ...
  -- Lock both coin rows.
  SELECT balance INTO _initiator_balance
    FROM public.user_coins WHERE user_id = _trade.initiator_id FOR UPDATE;
  SELECT balance INTO _responder_balance
    FROM public.user_coins WHERE user_id = _trade.responder_id FOR UPDATE;
  ...
  UPDATE public.user_inventory
     SET user_id = _trade.responder_id
   WHERE id = ANY(_initiator_items_arr);
  UPDATE public.user_inventory
     SET user_id = _trade.initiator_id
   WHERE id = ANY(_responder_items_arr);

  -- Charge gas fee.
  UPDATE public.user_coins
     SET balance = balance - _gas_fee, updated_at = now()
   WHERE user_id = _trade.initiator_id
   RETURNING balance INTO _new_initiator_balance;
  UPDATE public.user_coins
     SET balance = balance - _gas_fee, updated_at = now()
   WHERE user_id = _trade.responder_id
   RETURNING balance INTO _new_responder_balance;
  ...
  INSERT INTO public.trade_history(
    ...
    _gas_fee * 2, 'success', NULL, NULL, _caller_ip, _user_agent
  );
  ...
  RETURN jsonb_build_object(
    ...
    'new_balances', jsonb_build_object(
      'initiator', _new_initiator_balance,
      'responder', _new_responder_balance
    )
  );
END;
$$;

-- Lock down: revoke from anon/authenticated; only service role calls.
