-- 1. Add is_banned column to user_coins (existing per-user table)
ALTER TABLE public.user_coins
  ADD COLUMN IF NOT EXISTS is_banned boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS ban_reason text NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS banned_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_user_coins_is_banned ON public.user_coins(is_banned) WHERE is_banned = true;

-- 2. Find user by email (admin-only) — used to add admin/ban by email
...
-- 3. Admin-only: set ban status
CREATE OR REPLACE FUNCTION public.admin_set_user_banned(_user_id uuid, _banned boolean, _reason text DEFAULT '')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  UPDATE public.user_coins
     SET is_banned = _banned,
         ban_reason = CASE WHEN _banned THEN COALESCE(_reason, '') ELSE '' END,
         banned_at = CASE WHEN _banned THEN now() ELSE NULL END,
         updated_at = now()
   WHERE user_id = _user_id;

  RETURN jsonb_build_object('success', true, 'user_id', _user_id, 'banned', _banned);
END;
$$;
...
-- 4. Admin-only: list banned users with email
CREATE OR REPLACE FUNCTION public.admin_get_banned_users()
RETURNS TABLE(user_id uuid, email text, ban_reason text, banned_at timestamptz)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT uc.user_id, u.email::text, uc.ban_reason, uc.banned_at
...
REVOKE ALL ON FUNCTION public.admin_get_banned_users() FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_get_banned_users() TO authenticated;

-- 5. Block banned users in secure_draw
