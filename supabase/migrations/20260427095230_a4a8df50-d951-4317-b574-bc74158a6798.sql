...
-- Function: cek apakah user termasuk 100 pendaftar pertama (rate-up 1.5x)
...
BEGIN
  IF _user_id IS NULL THEN
    RETURN jsonb_build_object('is_rate_up', false, 'user_index', null, 'multiplier', 1.0, 'limit', _limit);
  END IF;
...
GRANT EXECUTE ON FUNCTION public.get_rate_up_status(uuid) TO authenticated, anon;

-- Helper: hitung multiplier untuk seorang user (dipakai di secure_draw)
...
DECLARE
  _index integer;
BEGIN
  IF _user_id IS NULL THEN RETURN 1.0; END IF;
  SELECT rn INTO _index FROM (
    SELECT id, ROW_NUMBER() OVER (ORDER BY created_at ASC, id ASC) AS rn
      FROM auth.users
...
-- Update secure_draw: kalikan probability weight tier S/A/B dengan multiplier user
...
BEGIN
  ...
  SELECT * INTO _coins FROM public.user_coins WHERE user_id = _user_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'no_coin_account'; END IF;
  IF _coins.is_banned THEN RAISE EXCEPTION 'user_banned'; END IF;
  IF _coins.last_draw_at IS NOT NULL AND _coins.last_draw_at > now() - interval '2 seconds' THEN
    RAISE EXCEPTION 'rate_limited';
  END IF;

  SELECT * INTO _campaign FROM public.campaigns WHERE id = _campaign_id AND is_active = true;
  ...
  IF _coins.balance < _total_cost THEN RAISE EXCEPTION 'insufficient_coins'; END IF;

  -- Rate-up multiplier untuk 100 user pertama
  _rate_mult := public._rate_up_multiplier(_user_id);
  ...
  FOR _i IN 1.._draw_count LOOP
    ...
    SELECT COALESCE(SUM(GREATEST(tp.total - tp.remaining, 0)), 0)
      ...
      FROM public.tier_prizes tp
      JOIN public.campaign_tiers ct ON ct.id = tp.tier_id
     WHERE ct.campaign_id = _campaign_id AND ct.label = 'C';
    ...
    IF _is_pity_draw THEN
      WITH active_tiers AS (
        SELECT ct.id, ct.label, ct.probability_weight
         ...
         WHERE ct.campaign_id = _campaign_id AND ct.label = _pity_tier
      ...
      SELECT id, label INTO _sel_tier_id, _sel_tier_label
        FROM active_tiers
       ORDER BY random() * probability_weight DESC LIMIT 1;
      IF _sel_tier_id IS NOT NULL THEN _has_pity := true; END IF;
    END IF;
    ...
    IF _sel_tier_id IS NULL THEN
      ...
      WITH active_tiers AS (
        SELECT ct.id, ct.label,
               (
                 ...
                 * CASE WHEN ct.label IN ('S','A','B') THEN _rate_mult ELSE 1.0 END
               ) AS effective_weight
          FROM public.campaign_tiers ct
          LEFT JOIN public.tier_prizes tp ON tp.tier_id = ct.id
         WHERE ct.campaign_id = _campaign_id
      ...
      SELECT id, label INTO _sel_tier_id, _sel_tier_label
        FROM cum WHERE cw >= random() * tw ORDER BY cw LIMIT 1;
    END IF;
    ...
    IF _selected_prize.is_digital THEN
      ...
      IF _code_row_id IS NULL THEN CONTINUE; END IF;
    END IF;
