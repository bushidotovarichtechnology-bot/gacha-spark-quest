-- Expose `total` (initial capacity) on public views so users can see total prize counts.
-- We keep `remaining` and `probability_weight` hidden — those remain admin-only.
